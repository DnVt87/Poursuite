...
For any conceptual archetype not represented in the operator's 10, **pick a representative public TJSP case to fill the gap.** Note explicitly in the inventory which cases are gap-fillers (not Poursuite cases) and which archetype each fills. Sealed and pre-2010 are the most likely gaps.

This is fine for two reasons: (a) eSAJ's unauthenticated consulta pública is *public by design* — the tribunal has already decided what strangers can see, and we're observing exactly that; (b) for sealed cases specifically, the seal-block enforces confidentiality on TJSP's side, so even probing one returns generic-only data. We're characterizing UI shapes, not bypassing access controls.

The goal is to surface every conditional rendering and every distinct field shape across the full diversity of TJSP execution cases, not just Poursuite's typical mix. Missing archetypes mean missing knowledge, which means surprises in production.
...
- Don't skip sealed cases when they appear in the operator's sample. We want to know what the seal-block rendering looks like even though our scraper currently bails on these.
- **Don't authenticate to eSAJ.** Run unauthenticated only. If a tab or element requires login (OAB credentials, digital cert), record "login wall observed at [tab/element]" as a first-class finding and move on. We're not using your brother's credentials for probe work — the risk-to-OAB and the data-quality-different-by-auth-level concerns both argue against it.

## On the methodology — clarifications

**"Every clickable" means every *type* of clickable, not exhaustive enumeration.** A case with 200 movimentos each linking to a document PDF doesn't need 200 entries in the inventory — it needs one entry per kind of link, with 1-2 representative examples and a count of how many of that kind exist. The catalog should be type-deep, not instance-deep.

**Headless vs. interactive DOM.** eSAJ may render differently under headless detection. Run headed for 2-3 cases as a sanity check, comparing to headless capture of the same cases. If identical, headless is fine for the rest. If they diverge, run the full inventory headed and record the divergence as a finding.

**Runtime — be thorough, time isn't the constraint.** This is the foundation of the system; the operator has explicitly said running 2-3 hours is fine if it produces a more complete inventory. Take the time the work needs. Quality over speed.
